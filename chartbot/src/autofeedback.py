...
class AutoFeedback:
    def __init__(self, gpt_api_key, claude_api_key, rendered_img, memory, data, run_only_gpt = False, d2c_model='ollama:granite4.1:3b'):
        self.memory = memory.copy()
        # Use the provided d2c_model (Ollama by default) for all LLM services
        self.af_gpt = LLM(model_name=d2c_model, api_key=None)
        self.af_claude = LLM(model_name=d2c_model, api_key=None)
        self.af_memory_gpt = []
    ...
    def semantic_decompose(self, data):
        ...
        # Safe extraction for JSON
        start_idx = gpt_query_response.find("{")
        end_idx = gpt_query_response.rfind("}")
        if start_idx != -1 and end_idx != -1 and start_idx < end_idx:
            gpt_query_response = gpt_query_response[start_idx : end_idx + 1]
            # Replace single quotes with double quotes but only outside of actual content if possible
            # But a simple regex approach is usually more robust for LLM badly formatted JSON:
            import ast
            try:
                # Try literal_eval first as it handles Python single-quoted dicts better than json.loads
                parsed_gpt = ast.literal_eval(gpt_query_response)
            except (ValueError, SyntaxError):
                try:
                    parsed_gpt = json.loads(gpt_query_response)
                except Exception:
                    # Fallback to basic string replacement and try JSON again
                    gpt_query_response = gpt_query_response.replace("'", '"')
                    try:
                        parsed_gpt = json.loads(gpt_query_response)
                    except:
                        parsed_gpt = {"Task": "Show Contradiction", "Purpose": "Show contradiction based on data.", "Audience": "General"}
        else:
            parsed_gpt = {"Task": "Show Contradiction", "Purpose": "Show contradiction based on data.", "Audience": "General"}
            
        self.memory['03_task(gpt4o)'] = parsed_gpt.get('Task', 'Show Contradiction')
        self.memory['03_purpose(gpt4o)'] = parsed_gpt.get('Purpose', '')
        self.memory['03_audience(gpt4o)'] = parsed_gpt.get('Audience', 'General')
        ...
        if not self.run_only_gpt:
            ...
            # Safe extraction for JSON
            start_idx = claude_query_response.find("{")
            end_idx = claude_query_response.rfind("}")
            if start_idx != -1 and end_idx != -1 and start_idx < end_idx:
                claude_query_response = claude_query_response[start_idx : end_idx + 1]
                import ast
                try:
                    parsed_claude = ast.literal_eval(claude_query_response)
                except (ValueError, SyntaxError):
                    try:
                        parsed_claude = json.loads(claude_query_response)
                    except Exception:
                        claude_query_response = claude_query_response.replace("'", '"')
                        try:
                            parsed_claude = json.loads(claude_query_response)
                        except:
                            parsed_claude = {"Task": "Show Contradiction", "Purpose": "Show contradiction based on data.", "Audience": "General"}
            else:
                parsed_claude = {"Task": "Show Contradiction", "Purpose": "Show contradiction based on data.", "Audience": "General"}

            self.memory['03_task(claude)'] = parsed_claude.get('Task', 'Show Contradiction')
            self.memory['03_purpose(claude)'] = parsed_claude.get('Purpose', '')
            self.memory['03_audience(claude)'] = parsed_claude.get('Audience', 'General')
